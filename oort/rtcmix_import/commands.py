try:
    from rtcmix import addgens, ampdb, autocorrect, boost, bt, bus_config, chans, constrain, control_rate, convolve, copygen, copytable, cos, cparam, cpath, cpslet, cpsmidi, cpsoct, cpspch, create_scale, dataset, dbamp, denoise, diapason, dumptable, dur, ellset, eqtemp, error, fdump, filechans, filedc, filedur, filepeak, filerms, filesr, fplot, freset, gaussrand, get_spray, getamp, getdry, getpch, getrange, getring, getwarp, grand, gravy, highrand, infile, info, init_filter, input, invertgen, irand, just, left_peak, ln, load, lowrand, lpcstuff, makeLFO, makeconnection, makeconverter, makefilter, makegen, makemonitor, makerandom, maketable, map, matrix, meantone, midipch, mikes, mikes_off, mod, modtable, mrotate, multgens, myscale, note_exists, numtest, octcps, octlet, octpch, offsetgen, oldmatrix, output, param, partch, path, path_p, pchadd, pchcps, pchlet, pchmidi, pchoct, peak, peakoff, pickrand, pickwrand, plottable, print_off, print_on, prob, punch, pythag, quantizegen, rand, random, rates, reset, resetamp, reversegen, right_peak, roomset, rotate, rtinput, rtoutput, rtsetparams, sampfunc, sampfunci, samptable, scalegen, set_attenuation_params, set_filter, set_hnfactor, set_option, set_thresh, setdev, setdevfactor, setdry, setexp, setline, setline_size, setrange, setring, setup, setwarp, setwindow, sfclean, sfcopy, sfprint, sgran, shiftgen, sin, space, speakerloc, speakerloc_p, spray_init, sr, srand, stgran, tablelen, tablemax, tablemin, tb, tbase, tempo, threshold, timeset, trand, translen, trirand, trunc, vMIX, wow, wrap, young
    dummy = False
except ImportError:
    dummy = True
    ...
    addgens = ampdb = autocorrect = boost = bt = bus_config = chans = constrain = control_rate = convolve = copygen = copytable = cos = cparam = cpath = cpslet = cpsmidi = cpsoct = cpspch = create_scale = dataset = dbamp = denoise = diapason = dumptable = dur = ellset = eqtemp = error = fdump = filechans = filedc = filedur = filepeak = filerms = filesr = fplot = freset = gaussrand = get_spray = getamp = getdry = getpch = getrange = getring = getwarp = grand = gravy = highrand = infile = info = init_filter = input = invertgen = irand = just = left_peak = ln = load = lowrand = lpcstuff = makeLFO = makeconnection = makeconverter = makefilter = makegen = makemonitor = makerandom = maketable = map = matrix = meantone = midipch = mikes = mikes_off = mod = modtable = mrotate = multgens = myscale = note_exists = numtest = octcps = octlet = octpch = offsetgen = oldmatrix = output = param = partch = path = path_p = pchadd = pchcps = pchlet = pchmidi = pchoct = peak = peakoff = pickrand = pickwrand = plottable = print_off = print_on = prob = punch = pythag = quantizegen = rand = random = rates = reset = resetamp = reversegen = right_peak = roomset = rotate = rtinput = rtoutput = rtsetparams = sampfunc = sampfunci = samptable = scalegen = set_attenuation_params = set_filter = set_hnfactor = set_option = set_thresh = setdev = setdevfactor = setdry = setexp = setline = setline_size = setrange = setring = setup = setwarp = setwindow = sfclean = sfcopy = sfprint = sgran = shiftgen = sin = space = speakerloc = speakerloc_p = spray_init = sr = srand = stgran = tablelen = tablemax = tablemin = tb = tbase = tempo = threshold = timeset = trand = translen = trirand = trunc = vMIX = wow = wrap = young = dummyCommand
